...
async def run_sync(full_scan=False):
	async with var_global.REPO_LOCK:
		...
		await report_sync(pushed, pulled, created)
...
async def report_sync(pushed, pulled, created):
	if not (pushed or pulled or created):
		return

	var_global.OPERATION_LOGGER.info(f'Sync complete - {len(created)} created, {len(pushed)} pushed, {len(pulled)} pulled')
